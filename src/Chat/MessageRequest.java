package Chat;

import java.io.Serializable;

/**
 * Create a request to send a message.
 *
 * @param <E>
 * @author Samuel Tregea
 * <p>
 * Last Date modified: January 2, 2019
 */
public class MessageRequest<E extends Serializable> implements Serializable {
    /**
     * The request type
     */
    private RequestType type;
    /**
     * The data associated with the request
     */
    private E data;

    /**
     * Create a new request.
     *
     * @param type request type
     * @param data the data
     */
    public MessageRequest(RequestType type, E data) {
        this.type = type;
        this.data = data;
    }

    /**
     * Get the type of request.
     *
     * @return request type
     */
    public RequestType getType() {
        return type;
    }

    /**
     * Get the data associated with the request.
     *
     * @return the data
     */
    public E getData() {
        return data;
    }

    /**
     * Enums used to determine the type of request
     */
    public enum RequestType {
        /**
         * After a successful client login, the server will send the current
         * chat room to the client.  This is only sent once - afterwards the
         * only information transmitted are the messages.
         */
        CHAT_ROOM,

        /**
         * A client's request to the server to change a tile.  It will contain
         * a Tile object.  It is important to note that the client should not
         * change the tile in their board until it is acknowledged by the server
         * via the TILE_CHANGED request.
         */
        SEND_MESSAGE,

        /**
         * Used for the server to tell the client there was an error.  It will
         * contain a message about the error. One place this is used is to tell
         * the client a login failed (because the username already exists).  It
         * is also used to indicate the server is shutting down, or any other
         * unusual things happen.
         */
        ERROR,

        /**
         * Used by the client to login to the server.  It will contain a string
         * that is the desired username for the client.
         */
        LOGIN,

        /**
         * Used by the server to indicate to the client the login succeeded.
         * It will contain a string indicating this.
         */
        LOGIN_SUCCESS,

        /**
         * Used by the server to indicate to all clients that a message has
         * officially been sent.  It will contain the new Chat.UserMessage object.
         * The clients should update their view of the board each time
         * a message arrives.
         */
        MESSAGE_SENT,

        /**
         * Used by the server to indicate to the client the login failed
         * due to a user using an empty string as their username.
         * It will contain a string indicating this.
         */
        INVALID_USERNAME,

        /**
         * Used by the server to indicate to the client the login failed
         * due to a user already having the same username.
         * It will contain a string indicating this.
         */
        USER_ALREADY_EXISTS

    }

}
